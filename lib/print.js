...
export default async function (m, conn = { user: {} }) {
  ...
  console.log(chalk.redBright('┍━━━━━━━━ ⋆⋅★彡[Ikai-Bot彡★⋅⋆ ━━━━━━━━━┑'))
...
${chalk.blueBright('┃')}
${chalk.magentaBright('┃')} 👤 ${chalk.yellowBright('Usuario:')} ${chalk.greenBright(user ? (user.wa_username ? '@' + user.wa_username + ' ' : '') + '|' + (user.exp || 0) + '|' + (user.coin || 0) : '|0|0')}
${chalk.redBright('┃')}
...
`)
  ...
  if (m.messageStubParameters) {
    console.log(m.messageStubParameters.map(jid => {
      ...
      return chalk.gray('+' + String(jid).replace('@s.whatsapp.net', '') + (name ? ' ~' + name : ''))
    }).join(', '))
  }
