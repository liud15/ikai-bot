        ...
        sendNCarousel: {
            async value(
                ...
                const interactiveMessage = {
                    ...
                    ...Object.assign({
                        mentions: typeof text === "string" ? await conn.parseMention(text || "@0", jid) : [],
                        contextInfo: {
                            mentionedJid: typeof text === "string" ?
                                await conn.parseMention(text || "@0", jid) :
                                [],
                            groupMentions: [],
...
export function protoType() {
    ...
    String.prototype.decodeJid = function decodeJid() {
        if (/:\d+@/gi.test(this)) {
            ...
            let jid = ((decode.user && decode.server && decode.user + "@" + decode.server) || this).trim();
            // Corrección de LIDs falsificados por Baileys (15 dígitos que terminan en @s.whatsapp.net)
            if (jid.endsWith('@s.whatsapp.net') && jid.split('@')[0].length >= 15 && /^[1-9]/.test(jid)) {
                return jid.replace('@s.whatsapp.net', '@lid');
            }
            return jid;
        } else {
            let jid = this.trim();
            // Corrección de LIDs falsificados por Baileys para JIDs normales
            if (jid.endsWith('@s.whatsapp.net') && jid.split('@')[0].length >= 15 && /^[1-9]/.test(jid)) {
                return jid.replace('@s.whatsapp.net', '@lid');
            }
            return jid;
