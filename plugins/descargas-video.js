...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  if (!text) return m.reply(`✳️ Ingresa el nombre del video.\nEjemplo: *${usedPrefix + command} Confess your love*`)
  ...
  try {
    ...
    const resolvedTitle = video.title || 'video'
    const fileName = sanitizeFilename(resolvedTitle) + '.mp4'

    let sendAsDoc = command === 'ytmp4'
...
乂  *Y T - V I D E O*
   ...
   ⭔  *Calidad* : ${result.medias.video.quality || '720p'}
   ⭔  *Tamaño* : ${fileSizeStr}
...
${global.wm || ''}
    ...
    console.error('Error en descargas-video:', e)
    m.reply(`❌ Ocurrió un error. Por favor, inténtalo de nuevo.\n*Error:* ${e.message}`)
  }
}

handler.help = ['ytvid <título>', 'playvideo <título>', 'ytmp4 <título>']
handler.tags = ['downloader']
handler.command = ['ytvid', 'playvideo', 'ytmp4']
handler.limit = true
handler.daftar = true
