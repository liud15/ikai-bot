...
let handler = async (m, { conn, command, text, usedPrefix }) => {
  ...
  if (/^logros$/i.test(command)) {
    const lines = ACHIEVEMENTS.map((a, i) => {
      ...
      const status = claimed ? '◈ Cobrado' : unlocked ? '𖤓 Disponible' : '◇ Bloqueado'
      return ` ◈ *${i + 1}. ${a.label}*\n   ▸ Premio : ${a.reward.toLocaleString()} coins\n   ▸ Estado : ${status}`
    }).join('\n\n')
  ...
  if (!Number.isInteger(idx) || !ACHIEVEMENTS[idx]) {
    return m.reply(
        `*₊˚ ✧ ‿︵‿୨ ୧‿︵‿ ✧ ˚₊*\n` +
        ` ❌ *COBRO DE LOGRO* ❌\n\n` +
        ` ⌲ Indica el número del logro que deseas cobrar.\n\n` +
  ...
  if (user.achievements[a.id]) return m.reply(
      `*₊˚ ✧ ‿︵‿୨ ୧‿︵‿ ✧ ˚₊*\n` +
      ` ⚠️ *LOGRO COBRADO* ⚠️\n\n` +
      ` ⌲ Ya has reclamado este premio anteriormente.\n` +
      `*₊˚ ✧ ‿︵‿୨ ୧‿︵‿ ✧ ˚₊*`
  )
  if (!a.check(user)) return m.reply(
      `*₊˚ ✧ ‿︵‿୨ ୧‿︵‿ ✧ ˚₊*\n` +
      ` 🔒 *LOGRO BLOQUEADO* 🔒\n\n` +
      ` ⌲ Aún no cumples los requisitos para este logro.\n` +
      `*₊˚ ✧ ‿︵‿୨ ୧‿︵‿ ✧ ˚₊*`
  )
  ...
  await conn.sendMessage(m.chat, {
    text: `*₊˚ ✧ ‿︵‿୨ ୧‿︵‿ ✧ ˚₊*\n` +
          ...
          ` ⌲ ${a.label}\n\n` +
          `   ▸ Premio : *+${a.reward.toLocaleString()} coins*\n` +
          `   ▸ Balance : *${user.coin.toLocaleString()}*\n` +
          `*₊˚ ✧ ‿︵‿୨ ୧‿︵‿ ✧ ˚₊*`,
    edit: loading.key
  })
}
