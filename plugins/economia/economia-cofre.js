...
let handler = async (m, { conn }) => {
  ...
  if (remaining > 0) return m.reply(
      `*₊˚ ✧ ‿︵‿୨ ୧‿︵‿ ✧ ˚₊*\n` +
      ` ⏳ *COFRE EN RECARGA* ⏳\n\n` +
      ` ⌲ Intenta de nuevo en *${msToText(remaining)}*.\n` +
      `*₊˚ ✧ ‿︵‿୨ ୧‿︵‿ ✧ ˚₊*`
  )
  ...
  if (roll < 0.68) {
    ...
    user.coin += coins
    result = `Recompensa : *${coins.toLocaleString()} coins*`
  } else if (roll < 0.93) {
    const exp = rand(220, 450)
    user.exp += exp
    result = `Recompensa : *${exp.toLocaleString()} exp*`
  } else {
    const diamonds = rand(2, 5)
    user.diamond += diamonds
    result = `Recompensa rara : *${diamonds.toLocaleString()} diamante(s)*`
  }
  ...
  await conn.sendMessage(m.chat, {
    text: `*₊˚ ✧ ‿︵‿୨ ୧‿︵‿ ✧ ˚₊*\n` +
          ` ✨ *COFRE ABIERTO* ✨\n\n` +
          ` ⌲ ${result}\n\n` +
          `   ▸ Coins Totales : *${user.coin.toLocaleString()}*\n` +
