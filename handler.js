...
async function areJidsSameUserLid(jid1, jid2, conn) {
    ...
    if (lid && pn && conn?.signalRepository?.lidMapping) {
        try {
            const resolvedPn = await conn.signalRepository.lidMapping.getPNForLID(lid);
            if (resolvedPn && pn.includes(resolvedPn.split('@')[0])) return true;
        } catch (e) { }
    }
    return false;
}
...
async function findParticipant(participants, jid, conn) {
    if (!participants || !jid) return null;
    let p = participants.find(v => v.id === jid);
    if (p) return p;
    
    if (jid.endsWith('@s.whatsapp.net')) {
        p = participants.find(v => v.phoneNumber && v.phoneNumber.includes(jid.split('@')[0]));
        if (p) return p;
    }
    
    if (jid.endsWith('@lid') && conn?.signalRepository?.lidMapping) {
        try {
            const resolvedPn = await conn.signalRepository.lidMapping.getPNForLID(jid);
            if (resolvedPn) {
                p = participants.find(v => v.id.includes(resolvedPn.split('@')[0]) || (v.phoneNumber && v.phoneNumber.includes(resolvedPn.split('@')[0])));
                if (p) return p;
            }
        } catch (e) { }
    }
...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        if (m.sender && (m.sender.endsWith('@lid') || (m.sender.endsWith('@s.whatsapp.net') && m.sender.split('@')[0].length >= 15))) {
            try {
                let realJid = null;
                // Intentar extraer el número real del mensaje (participantAlt)
                const ctx = m.message?.extendedTextMessage?.contextInfo || m.message?.imageMessage?.contextInfo || m.message?.videoMessage?.contextInfo || m.message?.audioMessage?.contextInfo || m.message?.documentMessage?.contextInfo;
                if (ctx?.participantAlt && ctx.participantAlt.endsWith('@s.whatsapp.net')) {
                    realJid = ctx.participantAlt;
                }
        ...
        if (!isROwner && m.sender.endsWith('@lid')) {
            for (const oj of ownerJids) {
                if (await areJidsSameUserLid(m.sender, oj, this)) { isROwner = true; break }
            }
        }
        ...
        const bot = (m.isGroup ? await findParticipant(participants, this.user?.jid || conn.user?.jid, this) : null) || {}
        ...
        if (chatData?.reaction && m.text?.match(/(ción|dad|aje|oso|izar|mente|pero|tion|age|ous|ate|and|but|ify|ahasto|mita|a|s)/gi)) {
